...
## Corpus Check
- 43 files · ~13,068 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 651 nodes · 1027 edges · 32 communities (25 shown, 7 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 230 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f0af7e2b`
...
Cohesion: 0.05
Nodes (27): RandomAgent, AppLogger, GameBoard, Return the Ship occupying this cell, or None., Process an incoming shot.         Returns (CellState.HIT, Ship) on hit, (CellSta, Return all (row, col) pairs not yet shot (EMPTY or ship still there)., Serialize board to a 10x10 list of 'SHIPTYPE:CELLSTATE' strings.         If fog_, Print the board to stdout with row/col headers. (+19 more)

### Community 2 - "Community 2"
Cohesion: 0.05
Nodes (31): BayesianAgent, Bayesian probability density agent.      Enumerates all valid ship placements on, FeatureExtractor, Return shape (4,) float32 global context vector., Computes per-cell (100, 16) and global (4,) features for the TransformerPPO agen, Record the outcome of a shot and propagate to the internal Bayesian model., Return shape (100, 16) float32 feature array for all cells.          Calls Bayes, parse_coordinate() (+23 more)
...
Cohesion: 0.06
Nodes (24): QAgent, Battleship agent backed by a Q-network (DQN).      Operates in greedy mode (epsi, Circular experience replay buffer for DQN training., ReplayBuffer, Transition, evaluate(), main(), parse_args() (+16 more)
...
## Knowledge Gaps
- **110 isolated node(s):** `Imitation pretraining: train TransformerPPONet to mimic BayesianAgent.  For each`, `One epoch of value-trunk-only updates — policy trunk receives zero gradient.`, `Run one episode, collecting transitions. Returns (transitions, turns_to_win).`, `Fast single-game Battleship environment for RL training.      Wraps GameBoard di`, `Take a shot at cell index `action` (row * 10 + col).          Returns:` (+105 more)
...
- **Why does `GameLogger` connect `Community 11` to `Community 0`, `Community 4`, `Community 6`, `Community 8`, `Community 10`, `Community 12`, `Community 15`, `Community 19`?**
  _High betweenness centrality (0.217) - this node is a cross-community bridge._
- **Why does `BayesianAgent` connect `Community 0` to `Community 11`, `Community 6`?**
  _High betweenness centrality (0.153) - this node is a cross-community bridge._
- **Why does `BattleshipEnv` connect `Community 1` to `Community 2`, `Community 5`?**
