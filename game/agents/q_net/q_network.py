...
class QNetwork(nn.Module):
    ...
    Args:
        cell_feats:   (B, 100, 3)  one-hot {unknown, hit, miss}
        global_feats: (B, 2)       [ships_sunk_norm, turn_norm]
    ...
    def forward(
        ...
        flat = cell_feats.flatten(start_dim=1)  # (B, 300)
        x = torch.cat([flat, global_feats], dim=1)  # (B, 302)
        q = self.net(x)  # (B, 100)
